...
#define core_dump()	core_dump_ex(__FILE__, __LINE__, __FUNCTION__)
    extern void		core_dump_ex(const char *who, long line, char const* function);	// ì½”ì–´ë¥¼ ê°•ì œë¡œ ëž¤í”„
...
#define TOKEN(string) if (!strcasecmp(token_string, string))
    ...
    extern uint32_t	thecore_random();				// ·£´ý ÇÔ¼ö
...
extern int		number_ex(int from, int to, const char *file, int line); // fromìœ¼ë¡œ ë¶€í„° toê¹Œì§€ìž˜ ëžœëž¤ ê°’ ë¦¬í„´
