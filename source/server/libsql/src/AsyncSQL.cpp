...
bool CAsyncSQL::Connect()
{
	...
	if (m_stHost.find("SOCKET") != std::string::npos)
    {
		...
		mysql_connection = mysql_real_connect(&m_hDB, nullptr, m_stUser.c_str(), m_stPassword.c_str(), m_stDB.c_str(), m_iPort, "/tmp/mysql.sock", CLIENT_MULTI_STATEMENTS);
	}
	else
#endif
    {
		mysql_connection = mysql_real_connect(&m_hDB, m_stHost.c_str(), m_stUser.c_str(), m_stPassword.c_str(), m_stDB.c_str(), m_iPort, nullptr, CLIENT_MULTI_STATEMENTS);
    }
...
SQLMsg* CAsyncSQL::DirectQuery(const char* c_pszQuery)
{
	...
	{
		sys_err("mysql_query(%s) failed with errno %u %s", p->stQuery.c_str(), mysql_errno(&m_hDB), mysql_error(&m_hDB));
