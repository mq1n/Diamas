...
int IODeviceProvider_File::read(void *buffer, unsigned int size, bool read_all)
{
	if (size == 0)
		return 0;
	if (peeked_data.get_size() > 0)
	{
		int peek_amount = std::min(size, peeked_data.get_size());
		...
		if (static_cast<unsigned int>(peek_amount) <= size)
...
{
	if (peeked_data.get_size() >= static_cast<unsigned int>(len))
	{
