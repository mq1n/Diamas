...
void AES256_Decrypt_Impl::set_key(const unsigned char key[32])
{
	cipher_key_set = true;
...
void AES256_Decrypt_Impl::add(const void *_data, int size)
{
	if (calculated)
		reset();
	...
	while (pos < size)
	{
		...
		int data_used = std::min(buffer_space, data_left);
...
bool AES256_Decrypt_Impl::calculate()
{
	...
	if (padding_enabled)
	{
		if (chunk_filled == aes256_block_size_bytes)
		{
			process_chunk();
			chunk_filled = 0;
			unsigned int current_size = databuffer.get_size();
			if (current_size > 0)
			{
