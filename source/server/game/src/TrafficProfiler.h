...
class TrafficProfiler : public singleton< TrafficProfiler >
{
	...
	public:
		...
		bool	Report( IODirection dir, uint8_t byHeader, uint32_t dwSize )
		{
			...
			if ( (uint32_t)(time( nullptr ) - m_tmProfileStartTime) >= m_dwFlushCycle )
				return Flush();
			return true;
		}
	...
	private:
		...
		uint32_t		m_dwFlushCycle;		///< Flush 주기
		time_t		m_tmProfileStartTime;	///< 프로파일을 시작한 시간. Flush 될 때마다 Update 된다.
		uint32_t		m_dwTotalTraffic;	///< Report 된 총 Traffic 용량
		uint32_t		m_dwTotalPacket;	///< Report 된 총 Packet 수
