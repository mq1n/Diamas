...
void CDungeon::SetFlag(const std::string &name, int32_t value)
{
...
void CDungeon::DecMember(LPCHARACTER ch)
{
	...
	if (m_set_pkCharacter.empty() && !CBattlegroundManager::Instance().IsEventMap(m_lMapIndex))
	{
...
void CDungeon::JumpGuild(CGuild* pGuild, int32_t lFromMapIndex, int32_t x, int32_t y)
{
	x *= 100;
	y *= 100;
	...
	if (pGuild->GetDungeon_for_Only_guild() == nullptr)
	{
		if (m_pGuild == nullptr)
		{
			m_pGuild = pGuild;
...
void CDungeon::PurgeUnique(const std::string& key)
{
	...
	if (it == m_map_UniqueMob.end())
	{
		sys_err("PurgeUnique - Unknown Key or Dead: %s", key.c_str());
		return;
	}
...
void CDungeon::KillUnique(const std::string& key)
{
	...
	if (it == m_map_UniqueMob.end())
	{
		sys_err("KillUnique - Unknown Key or Dead: %s", key.c_str());
		return;
	}
	LPCHARACTER ch = it->second;
	m_map_UniqueMob.erase(it);
...
uint32_t CDungeon::GetUniqueVid(const std::string& key)
{
	...
	if (it == m_map_UniqueMob.end())
	{
		sys_err("GetUniqueVid - Unknown Key or Dead: %s", key.c_str());
		return 0;
	}
	LPCHARACTER ch = it->second;
	return ch->GetVID();
...
float CDungeon::GetUniqueHpPerc(const std::string& key)
{
	...
	if (it == m_map_UniqueMob.end())
	{
		sys_err("GetUniqueHpPerc - Unknown Key : %s", key.c_str());
		return 0.f;
	}
...
bool CDungeon::IsUniqueDead(const std::string& key)
{
	...
	if (it == m_map_UniqueMob.end())
	{
		sys_err("IsUniqueDead - Unknown Key or Dead : %s", key.c_str());
		return true;
	}
...
LPCHARACTER CDungeon::SpawnGroup(uint32_t vnum, int32_t x, int32_t y, float radius, bool bAggressive, int32_t count)
{
	...
	while (count--)
	{
		LPCHARACTER chLeader = CHARACTER_MANAGER::Instance().SpawnGroup(vnum, m_lMapIndex, sx, sy, ex, ey, nullptr, bAggressive, this);
		if (chLeader && !ch)
			ch = chLeader;
	}
...
void CDungeon::SpawnRegen(const char* filename, bool bOnce)
{
	if (!filename)
	{
		sys_err("CDungeon::SpawnRegen(filename=nullptr, bOnce=%d) - m_lMapIndex[%d]", bOnce, m_lMapIndex); 
		return;
	}
...
void CDungeon::SpawnMoveGroup(uint32_t vnum, const char* pos_from, const char* pos_to, int32_t count)
{
	...
	while (count--)
	{
		...
		CHARACTER_MANAGER::Instance().SpawnMoveGroup(vnum, m_lMapIndex, pkSectreeMap->m_setting.iBaseX+ai.sx, pkSectreeMap->m_setting.iBaseY+ai.sy, pkSectreeMap->m_setting.iBaseX+ai.ex, pkSectreeMap->m_setting.iBaseY+ai.ey, tx, ty, nullptr, true);
	}
}
