...
int32_t main(int32_t argc, char* argv[])
{
	...
	FileSystem::Log(0, "FS unpacker started!");
	Sleep(1000);

	const auto indexFile = std::string("Index");
	auto secondArg = std::string(indexFile);
	...
	if (!std::count(secondArg.begin(), secondArg.end(), '.') && thirdArg.empty()) // if second arg has not contain dot(which is mean it's seems not like a file) swap args
	{
		...
		secondArg = indexFile; // second arg reverted to default index name
	}
...
checkIO:
	...
	if (!InitializeIndexFile(fs, secondArg, thirdArg, packs))
	{
		FileSystem::Log(LL_ERR, "InitializeIndexFile failed");
		return EXIT_FAILURE;
	}
