...
DataBuffer::DataBuffer(uint32_t new_size)
	: impl(std::make_shared<DataBuffer_Impl>())
{
	set_size(new_size);
}

DataBuffer::DataBuffer(const void *new_data, uint32_t new_size)
	: impl(std::make_shared<DataBuffer_Impl>())
{
	set_size(new_size);
...
uint32_t DataBuffer::get_size() const
{
	return impl->size;
}

uint32_t DataBuffer::get_capacity() const
{
	return impl->allocated_size;
}

char &DataBuffer::operator[](int32_t i)
{
	return impl->data[i];
}

const char &DataBuffer::operator[](int32_t i) const
{
	return impl->data[i];
}

char &DataBuffer::operator[](uint32_t i)
{
	return impl->data[i];
}

const char &DataBuffer::operator[](uint32_t i) const
{
	return impl->data[i];
}
...
void DataBuffer::set_size(uint32_t new_size)
{
...
void DataBuffer::set_capacity(uint32_t new_capacity)
{
