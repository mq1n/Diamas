...
{	
	template <class T, class ID = uint64_t>
	class IUniqueID
	{
	public:
		inline IUniqueID()
	...
	template<class T, class ID> ID
	IUniqueID<T, ID>::m_nextUniqueId = ID();

	template<class T, class ID> std::stack<ID>
	IUniqueID<T, ID>::m_freeUniqueId;

	template<class T, class ID> std::mutex
	IUniqueID<T, ID>::m_mutexUniqueId;
}
