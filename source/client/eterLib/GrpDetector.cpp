...
static int32_t CompareD3DDisplayModeOrder( const VOID* arg1, const VOID* arg2 )
{
...
uint32_t D3D_CAdapterDisplayModeList::GetDisplayModeNum()
{
	return m_uD3DDMNum;
}

uint32_t D3D_CAdapterDisplayModeList::GetPixelFormatNum()
{
	return m_uD3DFmtNum;
}


const D3DDISPLAYMODE&	D3D_CAdapterDisplayModeList::GetDisplayModer(uint32_t iD3DDM)
{
	assert(iD3DDM<m_uD3DDMNum);
	return m_akD3DDM[iD3DDM];
}

const D3DFORMAT&		D3D_CAdapterDisplayModeList::GetPixelFormatr(uint32_t iD3DFmt)
{
	assert(iD3DFmt<m_uD3DFmtNum);
	return m_aeD3DFmt[iD3DFmt];
}

VOID D3D_CAdapterDisplayModeList::Build(IDirect3D9& rkD3D, D3DFORMAT eD3DFmtDefault, uint32_t iD3DAdapterInfo)
{
	D3DDISPLAYMODE* akD3DDM=m_akD3DDM;
	D3DFORMAT* aeD3DFmt=m_aeD3DFmt;	

	uint32_t uD3DDMNum=0;
	uint32_t uD3DFmtNum=0;

	aeD3DFmt[uD3DFmtNum++]=eD3DFmtDefault;

	uint32_t uAdapterModeNum = rkD3D.GetAdapterModeCount(iD3DAdapterInfo, eD3DFmtDefault);
	for (uint32_t iD3DAdapterInfoMode=0; iD3DAdapterInfoMode<uAdapterModeNum; iD3DAdapterInfoMode++)
	{			
...
uint32_t D3D_CDeviceInfo::GetD3DModeInfoNum()
{
	return m_uD3DModeInfoNum;
}

D3D_SModeInfo* D3D_CDeviceInfo::GetD3DModeInfop(uint32_t iD3D_SModeInfo)
{
...
BOOL D3D_CDeviceInfo::FindDepthStencilFormat(IDirect3D9& rkD3D, uint32_t iD3DAdapterInfo, D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat, D3DFORMAT* pDepthStencilFormat )
{
    uint32_t m_dwMinDepthBits    = 16;
    uint32_t m_dwMinStencilBits  = 0;
...
BOOL D3D_CDeviceInfo::Build(IDirect3D9& rkD3D, uint32_t iD3DAdapterInfo, uint32_t iDevType, D3D_CAdapterDisplayModeList& rkD3DADMList, BOOL (*pfnConfirmDevice)(D3DCAPS9& rkD3DCaps, uint32_t uBehavior, D3DFORMAT eD3DFmt))
{	
	assert(pfnConfirmDevice!=nullptr && "D3D_CDeviceInfo::Build");
	...
	// GetFlagInfo
	{
		uint32_t uD3DFmtNum=rkD3DADMList.GetPixelFormatNum();

		for (uint32_t iFmt=0; iFmt<uD3DFmtNum; ++iFmt)
		{
			D3DFORMAT eD3DFmtPixel=rkD3DADMList.GetPixelFormatr(iFmt);		
			uint32_t dwD3DBehavior=0;
	...
	// BuildModeInfoList
	{
		uint32_t uD3DDMNum=rkD3DADMList.GetDisplayModeNum();
		uint32_t uD3DFmtNum=rkD3DADMList.GetPixelFormatNum();


		for (uint32_t iD3DDM=0; iD3DDM<uD3DDMNum; ++iD3DDM)
		{
...
BOOL D3D_CDeviceInfo::Find(uint32_t uScrWidth, uint32_t uScrHeight, uint32_t uScrDepthBits, BOOL isWindowed, uint32_t* piD3DModeInfo)
{
...
D3D_CDeviceInfo* D3D_CAdapterInfo::GetD3DDeviceInfop(uint32_t iD3DDevInfo)
{
...
D3D_SModeInfo* D3D_CAdapterInfo::GetD3DModeInfop(uint32_t iD3DDevInfo, uint32_t iD3D_SModeInfo)
{
...
BOOL D3D_CAdapterInfo::Find(uint32_t uScrWidth, uint32_t uScrHeight, uint32_t uScrDepthBits, BOOL isWindowed, uint32_t* piD3DModeInfo, uint32_t* piD3DDevInfo)
{
...
BOOL D3D_CAdapterInfo::Build(IDirect3D9& rkD3D, uint32_t iD3DAdapterInfo, PFNCONFIRMDEVICE pfnConfirmDevice)
{
	...
	for (uint32_t iDevType=0; iDevType<D3DDEVICETYPE_NUM; ++iDevType)
	{		
...
D3D_CAdapterInfo* D3D_CDisplayModeAutoDetector::GetD3DAdapterInfop(uint32_t iD3DAdapterInfo)
{
...
D3D_SModeInfo* D3D_CDisplayModeAutoDetector::GetD3DModeInfop(uint32_t iD3DAdapterInfo, uint32_t iD3DDevInfo, uint32_t iD3D_SModeInfo)
{	       
...
BOOL D3D_CDisplayModeAutoDetector::Find(uint32_t uScrWidth, uint32_t uScrHeight, uint32_t uScrDepthBits, BOOL isWindowed, uint32_t* piD3DModeInfo, uint32_t* piD3DDevInfo, uint32_t* piD3DAdapterInfo)
{
...
BOOL D3D_CDisplayModeAutoDetector::Build(IDirect3D9& rkD3D, PFNCONFIRMDEVICE pfnConfirmDevice)
{
	...
	uint32_t uTotalAdapterCount=rkD3D.GetAdapterCount();	
	uTotalAdapterCount=std::min<uint32_t>(uTotalAdapterCount, D3DADAPTERINFO_NUM);

	for (uint32_t iD3DAdapterInfo=0; iD3DAdapterInfo<uTotalAdapterCount; ++iD3DAdapterInfo)
	{
