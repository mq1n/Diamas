...
void CCamera::SetViewMatrix()
{
	...
	m_fPitch = D3DXVec3Dot(&m_v3Up, &tmp); // / D3DXVec2Length(&v2ViewYZ);
	if (m_fPitch >= 1)
		m_fPitch = 1;
	else if (m_fPitch <= -1)
		m_fPitch = -1;
	...
	tmp = (v3CenterRay - m_fTerrainCollisionRadius * m_v3Up);
	...
	tmp = (v3CenterRay + m_fObjectCollisionRadius * m_v3Cross);
	...
	m_kRightObjectCollisionRay.SetDirection(v3CenterRay - m_fObjectCollisionRadius * m_v3Cross,
		D3DXVec3Length(&tmp));
	tmp = (v3CenterRay + m_fObjectCollisionRadius * m_v3Up);
...
//		return;
	...
	auto tmp = (v3Temp + m_v3Up);
	D3DXVec3TransformCoord(&m_v3Up, &tmp, &matRot);
...
void CCamera::CalculateRoll()
{
	...
	auto pV2 = D3DXVECTOR2(0.0f, 1.0f);
	float fDot = D3DXVec2Dot(&v2ViewXY, &pV2);
	if (fDot >= 1)
		fDot = 1;
	else if (fDot <= -1)
		fDot = -1;
	fDot = acosf(fDot);
	fDot *= (180.0f / D3DX_PI);
	pV2 = D3DXVECTOR2(0.0f, 1.0f);
	float fCross = D3DXVec2CCW(&v2ViewXY, &pV2);
	if ( 0 > fCross)
		fDot = -fDot;
