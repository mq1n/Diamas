...
//		return false;
	...
	uint32_t dwCurTime = GetCurrentTime();
	uint32_t dwElapsedTime = dwCurTime - m_dwStartTime;
...
// 	Tracef("%f, %f, %f, %f\n", fCurRed, fCurGreen, fCurBlue, fCurAlpha);
	...
	m_dwCurColor = (((uint32_t)(fCurAlpha * 255.0f)&0xff)<< 24) |
		(((uint32_t)(fCurRed * 255.0f)&0xff) << 16) |
		(((uint32_t)(fCurGreen * 255.0f)&0xff) << 8) |
		((uint32_t)(fCurBlue * 255.0f)&0xff);
