...
AES256_Decrypt_Impl::AES256_Decrypt_Impl() : initialisation_vector_set(false), cipher_key_set(false), padding_enabled(true), padding_pkcs7(true)
{
	reset();
}
...
void AES256_Decrypt_Impl::reset()
{
	calculated = false;
...
void AES256_Decrypt_Impl::set_iv(const uint8_t iv[16])
{
...
void AES256_Decrypt_Impl::set_padding(bool value, bool use_pkcs7)
{
	padding_enabled = value;
	padding_pkcs7 = use_pkcs7;
}

void AES256_Decrypt_Impl::set_key(const uint8_t key[32])
{
	cipher_key_set = true;
	extract_encrypt_key256(key, key_expanded);
	extract_decrypt_key(key_expanded, aes256_num_rounds_nr);
}

void AES256_Decrypt_Impl::add(const void *_data, int32_t size)
{
	if (calculated)
		reset();
...
bool AES256_Decrypt_Impl::calculate()
{
	if (calculated)
		reset();
	...
	if (padding_enabled)
	{
		if (chunk_filled == aes256_block_size_bytes)
		{
			process_chunk();
			chunk_filled = 0;
			uint32_t current_size = databuffer.get_size();
			if (current_size > 0)
			{
				uint8_t *dest_ptr = (uint8_t *)databuffer.get_data();
				uint32_t pad_byte = dest_ptr[current_size - 1];
				if (padding_pkcs7)
				{
					if ((pad_byte == 0) || (pad_byte > aes256_block_size_bytes))
					{
...
void AES256_Decrypt_Impl::process_chunk()
{
	...
	// Round 12
	s0 = table_d0[t0 >> 24] ^ table_d1[(t3 >> 16) & 0xff] ^ table_d2[(t2 >> 8) & 0xff] ^ table_d3[t1 & 0xff] ^ key_expanded_ptr[48];
	s1 = table_d0[t1 >> 24] ^ table_d1[(t0 >> 16) & 0xff] ^ table_d2[(t3 >> 8) & 0xff] ^ table_d3[t2 & 0xff] ^ key_expanded_ptr[49];
	s2 = table_d0[t2 >> 24] ^ table_d1[(t1 >> 16) & 0xff] ^ table_d2[(t0 >> 8) & 0xff] ^ table_d3[t3 & 0xff] ^ key_expanded_ptr[50];
	s3 = table_d0[t3 >> 24] ^ table_d1[(t2 >> 16) & 0xff] ^ table_d2[(t1 >> 8) & 0xff] ^ table_d3[t0 & 0xff] ^ key_expanded_ptr[51];
	// Round 13
	t0 = table_d0[s0 >> 24] ^ table_d1[(s3 >> 16) & 0xff] ^ table_d2[(s2 >> 8) & 0xff] ^ table_d3[s1 & 0xff] ^ key_expanded_ptr[52];
	t1 = table_d0[s1 >> 24] ^ table_d1[(s0 >> 16) & 0xff] ^ table_d2[(s3 >> 8) & 0xff] ^ table_d3[s2 & 0xff] ^ key_expanded_ptr[53];
	t2 = table_d0[s2 >> 24] ^ table_d1[(s1 >> 16) & 0xff] ^ table_d2[(s0 >> 8) & 0xff] ^ table_d3[s3 & 0xff] ^ key_expanded_ptr[54];
	t3 = table_d0[s3 >> 24] ^ table_d1[(s2 >> 16) & 0xff] ^ table_d2[(s1 >> 8) & 0xff] ^ table_d3[s0 & 0xff] ^ key_expanded_ptr[55];
...
void AES256_Decrypt::set_iv(const uint8_t iv[16])
{
	impl->set_iv(iv);
}

void AES256_Decrypt::set_key(const uint8_t key[32])
{
	impl->set_key(key);
}
...
void AES256_Decrypt::add(const void *data, int32_t size)
{
	impl->add(data, size);
}
