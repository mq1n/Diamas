...
void RSA_Impl::generate_prime(Random &random, BigInt &prime, int32_t prime_len)
{
	...
	uint8_t *random_bytes_ptr = random_key.get_data();	// Ready to write to
...
void RSA_Impl::pkcs1v15_encode(int32_t block_type, Random &random, const char *msg, int32_t mlen, char *emsg, int32_t emlen)
{
	...
	*(emsg++) = block_type;
	...
	case 1:
		...
		break;
	case 2:
		random.get_random_bytes_nzero((uint8_t *)emsg, length);	// generate random pad
		break;
	default:
...
Secret RSA_Impl::pkcs1v15_decode(const char *emsg, int32_t emlen)
{
	...
	for (ix = 2 + 8; ix < emlen; ix++)		// "As least 8 octets int32_t PS"
		if (emsg[ix] == 0)
			break;
...
{
	int32_t k = modulus->unsigned_octet_size();	// length of modulus, in bytes
