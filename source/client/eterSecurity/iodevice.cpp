...
int32_t IODevice::get_size() const
{
	if (impl)
...
int32_t IODevice::get_position() const
{
	if (impl)
...
int32_t IODevice::send(const void *data, int32_t len, bool send_all)
{
	if (impl)
...
int32_t IODevice::receive(void *data, int32_t len, bool receive_all)
{
	if (impl)
...
int32_t IODevice::peek(void *data, int32_t len)
{
	if (impl)
...
bool IODevice::seek(int32_t position, SeekMode mode)
{
	if (impl)
...
int32_t IODevice::read(void *data, int32_t len, bool receive_all)
{
	return receive(data, len, receive_all);
}

int32_t IODevice::write(const void *data, int32_t len, bool send_all)
{
...
std::string IODevice::read_string_text(const char *skip_initial_chars, const char *read_until_chars, bool allow_eof)
{
	...
	if (skip_initial_chars)
	{
		while (find_flag)	// For the entire file
		{
			...
			char *dptr = buffer;
			for (int32_t cnt = 0; cnt < read_size; cnt++, dptr++)	// Search the buffer
			{
				char letter = *dptr;
	...
	while (find_flag)
	{
		read_size = receive(buffer, buffer_size, true);
		char *dptr = buffer;
		for (int32_t cnt = 0; cnt < read_size; cnt++, dptr++)
		{
			char letter = *dptr;
