...
std::string CAnticheatManager::GetFileMd5(const std::string & szName)
{
	...
	CFile pMappedFile;
	if (!FileSystemManager::Instance().OpenFile(szName, pMappedFile))
		return szOutput;

	szOutput = GetMd5(reinterpret_cast<const uint8_t*>(pMappedFile.GetData()), pMappedFile.GetSize());
...
void CAnticheatManager::ExitByAnticheat(uint32_t dwErrorCode, uint32_t dwSubCode1, uint32_t dwSubCode2, bool bForceClose, const std::string& stSubMessage)
{
	...
	{
		auto stMessage = xorstr("Anticheat violation detected! Error code: ").crypt_get() +
			std::to_string(dwErrorCode) + "-" + std::to_string(dwSubCode1) + "-" + std::to_string(dwSubCode2) +
			" " + stSubMessage;

		TraceError(stMessage.c_str());
		ServiceMessageBox(xorstr("Anticheat").crypt_get(), stMessage, MB_ICONERROR);
		__fastfail(-1);
		// abort();
	}
