...
void AES_Impl::extract_encrypt_key128(const uint8_t key[aes128_key_length_bytes], uint32_t key_expanded[aes128_nb_mult_nr_plus1])
{
	for (int32_t cnt = 0; cnt < aes128_key_length_nk; cnt++)
	{
	...
	for (int32_t cnt = 0; cnt < aes128_num_rounds_nr; cnt++)
	{
...
void AES_Impl::extract_encrypt_key192(const uint8_t key[aes192_key_length_bytes], uint32_t key_expanded[aes192_nb_mult_nr_plus1])
{
	for (int32_t cnt = 0; cnt < aes192_key_length_nk; cnt++)
	{
		key_expanded[cnt] = get_word(key + 4 * cnt);
	}
...
void AES_Impl::extract_encrypt_key256(const uint8_t key[aes256_key_length_bytes], uint32_t key_expanded[aes256_nb_mult_nr_plus1])
{
	for (int32_t cnt = 0; cnt < aes256_key_length_nk; cnt++)
	{
		key_expanded[cnt] = get_word(key + 4 * cnt);
	}
...
void AES_Impl::store_block(uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3, DataBuffer &databuffer)
{
	...
	// Store the data
	int32_t current_size = databuffer.get_size();
	int32_t current_capacity = databuffer.get_capacity();
	int32_t available = current_capacity - current_size;
...
void AES_Impl::extract_decrypt_key(uint32_t *key_expanded, int32_t num_rounds)
{
	// Invert the order of the round keys
	for (int32_t start_value = 0, end_value = 4 * num_rounds; start_value < end_value; start_value += 4, end_value -= 4)
	{
		uint32_t temp;
	...
	uint32_t *key_expanded_ptr = key_expanded;
	for (int32_t cnt = 1; cnt < num_rounds; cnt++)
	{
		key_expanded_ptr += 4;
		key_expanded_ptr[0] =
...
void AES_Impl::create_tables()
{
	...
	for (cnt = 0; cnt < 256; cnt++)
	{
		...
		int32_t a = mul_value(0xe, x, power, logarithm);
		int32_t b = mul_value(0x9, x, power, logarithm);
		int32_t c = mul_value(0xd, x, power, logarithm);
		int32_t d = mul_value(0xb, x, power, logarithm);
