...
{
	printf("PID: %d\t", (int32_t)hSourcePid);
	printf("Object %p\t", pObject);
	printf("Handle %p\r\n", hHandle);

	HANDLE hProcess = OpenProcess(PROCESS_DUP_HANDLE | PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, (uint32_t)hSourcePid);
...
bool ScanHandles()
{
	...
	while ((ntStat = NtQuerySystemInformation(SystemExtendedHandleInformation, pHandleInfo, dwHandleInfoSize, nullptr)) == STATUS_INFO_LENGTH_MISMATCH)
	{
		dwHandleInfoSize *= 2;
