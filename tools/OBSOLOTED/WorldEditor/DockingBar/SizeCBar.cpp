...
{
	m_bFloatFixedWidth = FALSE;
	m_bDockingFixedWidth = FALSE;
...
CSize CSizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	// SHIN
    if (m_bFloatFixedWidth)	m_szFloat.cx = max(0, m_szMin.cx);
	else m_szFloat.cx = max(m_szFloat.cx, m_szMin.cx);
...
void CSizingControlBar::StartTracking(UINT nHitTest)
{
    ...
    else
    {
		...
		int index;
        for (index = 0; index < arrSCBars.GetSize(); index++)
            if (arrSCBars[index] == this) break;

        CSizingControlBar* pBar = arrSCBars[index +
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
	...
	// SHIN
	if (m_bDockingFixedWidth) szDelta.cx = 0;
...
BOOL CSizingControlBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
    ...
    int nMinLength = 2;
    int nWidth = 0;
    for (int iBar = 0; iBar < arrSCBars.GetSize(); iBar++)
    {
        pBar = arrSCBars[iBar];
    ...
    for (int jBar = 0; jBar < arrSCBars.GetSize(); jBar++)
	{
        if (bHorz)
            arrSCBars[jBar]->m_szHorz.cy = nWidth;
        else
            arrSCBars[jBar]->m_szVert.cx = nWidth;
	}
...
// SHIN
void CSizingControlBar::SetFloatFixedWidth(BOOL bFlag)
{
	m_bFloatFixedWidth = bFlag;
}

void CSizingControlBar::SetDockingFixedWidth(BOOL bFlag)
{
	m_bDockingFixedWidth = bFlag;
}

void CSizingControlBar::SetMinSize(CSize szMin)
{
	m_szMin = szMin;	
}
