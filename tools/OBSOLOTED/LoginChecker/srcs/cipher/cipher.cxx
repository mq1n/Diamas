...
size_t Cipher::Prepare(char* buffer, int *length) {
  assert(key_agreement_ == NULL);
  key_agreement_ = new DH2KeyAgreement();
  assert(key_agreement_ != NULL);
  size_t agreed_length = key_agreement_->Prepare((void*)buffer, (size_t*)length);
  if (agreed_length == 0) {
    delete key_agreement_;
    key_agreement_ = NULL;
  }
...
bool Cipher::Activate(bool polarity, size_t agreed_length,
  ...
  bool result = false;
  if (key_agreement_->Agree(agreed_length, (void*)buffer, length)) {
